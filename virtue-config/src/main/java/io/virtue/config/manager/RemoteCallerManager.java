...
public class RemoteCallerManager extends AbstractManager<List<RemoteCaller<?>>> {
    ...
    public <T> RemoteCaller<T> get(Class<T> interfaceType) {
        if (interfaceType.isAnnotationPresent(io.virtue.config.annotation.RemoteCaller.class)) {
            io.virtue.config.annotation.RemoteCaller annotation = interfaceType.getAnnotation(io.virtue.config.annotation.RemoteCaller.class);
