...
public class RemoteServiceManager extends AbstractManager<RemoteService<?>> {
    ...
    public <T> RemoteService<T> get(Class<T> interfaceType) {
        if (interfaceType.isAnnotationPresent(io.virtue.config.annotation.RemoteService.class)) {
            io.virtue.config.annotation.RemoteService annotation = interfaceType.getAnnotation(io.virtue.config.annotation.RemoteService.class);
