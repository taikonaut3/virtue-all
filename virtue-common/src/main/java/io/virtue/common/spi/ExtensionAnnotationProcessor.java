...
public class ExtensionAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        Set<? extends Element> serviceInterfaceElements = roundEnvironment.getElementsAnnotatedWith(Extension.class);
            ...
            if (element instanceof TypeElement typeElement) {
                List<? extends AnnotationMirror> annotationMirrors = typeElement.getAnnotationMirrors().stream()
                        .filter(annotationMirror ->
                                annotationMirror.getAnnotationType().toString().equals(EXTENSION_NAME))
                        .toList();
                ExtensionWrapper wrapper = new ExtensionWrapper(annotationMirrors.get(0));
    ...
    private List<TypeMirror> getAllInterfaces(TypeElement typeElement) {
        List<? extends TypeMirror> typeMirrors = typeElement.getInterfaces().stream().filter(typeMirror -> {
            ...
            Extensible annotation = interfaceElement.getAnnotation(Extensible.class);
    ...
    private static class ExtensionWrapper {
        ...
        ExtensionWrapper(AnnotationMirror annotationMirror) {
            ...
            for (ExecutableElement key : elementValues.keySet()) {
                ...
                if (key.getSimpleName().toString().equals("interfaces")) {
                    ...
                    values = (List<String>) annotationValue.getValue();
                }
            }
